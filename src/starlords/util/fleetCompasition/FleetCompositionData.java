...
public class FleetCompositionData {
    HashMap<String,ShipCompositionData> data = new HashMap<>();
    HashMap<String,Integer> numShips = new HashMap<>();//this is for storing the number of each ship active in the fleet.
    /*todo: so... how the hell is this even going to work?
            ...
            in each ship I am going to need:
                1) a special memCompressedData of some type donated to ships.
                ...
                2) a specal memCompressedData that is only held on the fleet:
    ...
    public ShipCompositionData getCurrentToBuildShip(){
        //todo: having this here might not make mush sense. but then again, I do need an overriding script maybe???
        //      I am so confused...
        double priority = Double.MIN_VALUE;
        ShipCompositionData out = null;
        for (String a : data.keySet()){
            ShipCompositionData ship = data.get(a);
            double p = ship.getPriorityOverriderByMinMax(this,ship.getPriorityToBuild(this));
            if (p > priority){
                out = ship;
                priority = p;
            }
